...
original_author="sambacha"
original_name="flask_web3"
original_urlname="flask-web3"
original_description="Awesome flask_web3 created by sambacha" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_web3 "${name}"
...
then
    ...
    echo "then run 'flask_web3' or 'python -m flask_web3'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_web3' or 'python -m flask_web3' or 'poetry run flask_web3'"
fi
